/** includes libkeccak-tiny by David Leon Gil **/
#include <stdint.h>
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
unsigned char * take_bin(unsigned int length){
    ...
    for(i = 0; i<length; i++) buffer[i] = take_char();
    return buffer;
}

void put_bin( unsigned char * bin, uint32_t bin_len) {
    uint32_t i;
    for (i=0;i<bin_len;i++) put_char(bin[i]);
}
...
while (1) {
    input_size = take_int();
    if (input_size != 0) {
        input_buffer = take_bin(input_size);
        if ( !input_buffer || sha3_256(checksum, 32, input_buffer, input_size)) _exit (1);
        free(input_buffer);
    }
    else { sha3_256(checksum, 32, checksum, 0); }
    put_int(32);
    put_bin(checksum, 32);
    fflush(stdout);
